...
def ant(ctxt, file_=None, target=None, keep_going=False, args=None):
    ...
    logfile = tempfile.NamedTemporaryFile(prefix='ant_log', suffix='.xml', dir=ctxt.basedir)
    logfile.close()
    fixedLogfileName = os.popen('cygpath --windows ' + logfile.name, 'r').read().rstrip('\n')
    if args:
    ...
    args += ['-noinput', '-listener', 'org.apache.tools.ant.XmlLogger',
             ...
             '-DXmlLogger.file', fixedLogfileName]
    if file_:
    ...
    try:
        xml_log = xmlio.parse(file(fixedLogfileName, 'r'))

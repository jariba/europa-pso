...
/*
 * AshConsole.java - jEdit's text component
 *
...
/**
 * AshConsole - a shell for Antlr Grammars, based on jEdit's text area component.
 * From the JEdit API docs:
 * It is more suited for editing program source code than JEditorPane, because
 * it drops the unnecessary features (images, variable-width lines, and so on)
 * and adds a whole bunch of useful goodies such as:
 * <ul>
 ...
 * <pre>AshConsole ta = new AshConsole();
...
public class AshConsole extends JComponent {
  ...
  protected static AshConsole focusedComponent;
  ...
  protected boolean rectSelect;
	...
	protected static float lineSpacing = 1.2f;
  protected static float lineDescent = (lineSpacing-1f) * .5f + 1f;

  private LinkedList history = new LinkedList();
  private String currCommand = "";
  private int historyCursor = 0;
  private AshInterpreter interp = null;

  /**
   * Creates a new AshConsole with the default settings.
   */
  public AshConsole(AshInterpreter interp) {
    this(ConsoleDefaults.getDefaults(), interp);
  }

  /**
   * Creates a new AshConsole with the specified settings.
   * @param defaults The default settings
   */
  public AshConsole(ConsoleDefaults defaults, AshInterpreter interp) {
    this.interp = interp;
    ...
    painter = new ConsolePainter(this,defaults);
   ...
   */
  public final ConsolePainter getPainter() {
    return painter;
  }
  ...
  public final void recalculateVisibleLines() {
    ...
    int lineHeight = (int) (painter.getFontMetrics().getHeight() * lineSpacing);
  ...
  public int lineToY(int line) {
    ...
    return (line - firstLine) * (int) (fm.getHeight()*lineSpacing)
  ...
  public final void printBanner() {
    try {
      document.insertString(document.getLength(),interp.getBanner(),null);
      document.insertString(document.getLength(),"\n",null);
      document.insertString(document.getLength(),interp.getPrompt(),null);
    }
    catch(BadLocationException bl) {
      bl.printStackTrace();
      return;
    }
    commandStart = document.getLength();
  }

  public final void upHistory() {
    try {
      if(historyCursor == history.size())
        currCommand = document.getText(commandStart, document.getLength()-commandStart);
    }
    ...
    if(historyCursor <= 0) {
      ...
      --historyCursor;
    String historyElement = (String) history.get(historyCursor);
    select(commandStart, document.getLength());
    overwriteSetSelectedText(historyElement);
  }

  public final void downHistory() {
    if(historyCursor >= history.size()) {
      getToolkit().beep();
      return;
    }
      ...
      ++historyCursor;
    String historyElement = currCommand;
    if(historyCursor < history.size())
      historyElement = (String) history.get(historyCursor);
    select(commandStart, document.getLength());
    overwriteSetSelectedText(historyElement);
  }

  public final String acceptCommand() {
    // get current command
    String command = getText(commandStart, document.getLength()-commandStart);
    try {
      document.insertString(document.getLength(),"\n", null);
      // try running command
  ...
  public final boolean runCommand(String command) {
  ...
  public final void selectNone() {
    select(getDocumentLength(), getDocumentLength());
  }
  ...
  public void setSelectedText(String selectedText) {
    ...
    try {
      if(selectionStart < commandStart)
        setCaretPosition(commandStart);
      if(rectSelect) {
        Element map = document.getDefaultRootElement();
  ...
  class FocusHandler implements FocusListener {
    public void focusGained(FocusEvent evt) {
      ...
      focusedComponent = AshConsole.this;
    }
  ...
  class MouseWheelHandler implements MouseWheelListener {
    public void mouseWheelMoved(MouseWheelEvent evt) {
      ...
      if(evt.getScrollType() == evt.WHEEL_UNIT_SCROLL)
        newFirstLine = Math.max(0, Math.min(getLineCount()-visibleLines, firstLine+evt.getWheelRotation()));
      else
        newFirstLine = Math.max(0, Math.min(getLineCount()-visibleLines, firstLine+evt.getWheelRotation()*5));

      vertical.setValues(newFirstLine,visibleLines,0,getLineCount());
      // If this is not done, mousePressed events accumilate
      // and the result is that scrolling doesn't stop after
      // the mouse is released
      SwingUtilities.invokeLater(new Runnable() {

...
public class DefaultInputHandler extends InputHandler {
  ...
  public void addDefaultKeyBindings() {
    ...
    addKeyBinding("ENTER",NEW_LINE);
    addKeyBinding("A+ENTER",INSERT_BREAK);
    //addKeyBinding("C+ENTER",REPEAT);

    if(Toolkit.getDefaultToolkit().getMenuShortcutKeyMask() == KeyEvent.META_MASK) {
      addKeyBinding("M+X",CUT_SELECTION);
      addKeyBinding("M+C",COPY_SELECTION);
  ...
  public void keyTyped(KeyEvent evt) {
    ...
    if(c != KeyEvent.CHAR_UNDEFINED &&
        (modifiers & (KeyEvent.ALT_MASK | KeyEvent.META_MASK)) == 0) {
      if(c >= 0x20 && c != 0x7f) {

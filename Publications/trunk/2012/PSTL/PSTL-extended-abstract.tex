...
\europa\ is designed to be open and extendable to accommodate diverse and highly specialized problem solving techniques within a common design framework and around a common technology core; Figure~\ref{fig:architecture} shows the main components of \europa\ and the relationships/interactions between them. It is now at version 2.6 and is the successor of the original \europa\ which in turn was based upon HSTS ~\cite{muscettola:1998}. It has been made available under an open-source license. The source code and extensive documents on \europa\ are available at: \emph{http://code.google.com/p/europa\-pso/}. 


%----------
\section{Timeline-based Planning}

\noindent {\bf Modeling:} The NDDL input modeling language representation includes state and activity descriptions, as is common in planners using traditional modeling languages like the Planning Domain Definition Language (PDDL) utilizing the variable-value formalism. \europa\ thus takes its heritage from planning formalisms like IxTeT and SAS+. The NDDL model are parsed/translated into an internal representation with the following core components:

\begin{itemize}
\item {\bf Timeline \& Token:} \europa\ state variables are represented using \emph{timelines}, and the changing values on timelines represent sequences of states. Specifically, states are represented as a set of temporally extended predicates and resource usage, called \emph{tokens} and the temporal and logical constraints between them. Each token consists of a proposition and a list of parameters, which by default includes the \emph{start}, \emph{end} and \emph{duration} temporal variables.  Timelines consist of totally ordered sequences of connected tokens; hence, a timeline can be in only one state at any instant. Note that while tokens mostly represent temporally-extended predicates, which resemble world-state, tokens also represent actions in \europa\ and thus may not be on some particular timelines.
...
\noindent{\bf Least-Commitment Partial Planning Refinement Search:} EUROPA follows the \emph{plan-space refinement} planning approach. It uses a series of refinements to convert an initial partial plan into a final plan that is complete with respect to the requirements of the planner. A partial plan consist of a set of timelines containing partially instantiated and ordered set of tokens with the possible flaws of: (1) unbound variables; (2) open condition; and (3) temporal threats.  Flaws are resolved one at a time following user-defined flaw filter and flaw selection strategy and a complete plan is returned when there is no flaw. Below, we outline the flaw types and the resolution strategy for each of them:
...
\item {\bf Open Condition:} when a new token is added to the partial-plan, its status is \emph{inactive} and it represents an open-condition flaw. This type of flaw can be resolved by: (1) \emph{merge} with an existing active token; (2) \emph{activate} and add to timeline; and (3) \emph{reject} (if this option is allowed). When a token is ``activated", it can introduce a new set of (inactive) \emph{slave} tokens. For example, if the activated token represents a durative action, then (slave) tokens representing (pre)conditions and effects are added as new inactive tokens (new open conditions).
\item {\bf Threat:} once a token has been placed in the partial plan it may impact other tokens indirectly through possible overlapping requirements on objects. Threats are resolved by imposing ordering constraints among tokens.
\end{itemize}

There are extendable built-in flaw-filter and flaw-selection strategies to decide which flaw to be handled next. When a flaw is selected, it represents a branching point in the chronological backtracking (depth-first) search graph with the next child search node to be selected/explored basing on the resolution-selection strategy (which \europa\ also provides extendable built-in options). The flaw-resolution strategy is backed by strong constraint propagation routines with special emphasis on temporal and resource reasoning.The search process stops when there is no additional flaw to handle.

%---------
\section{Current State \& Future Work}

\europa\ and its supporting tools have been going through a long period of development. Besides the core modeling and reasoning capabilities, \europa\ also provides strong API to integrate with native client applications, Eclipse's plugin to help build and debug NDDL/ANML models, and visualization capability to support plan comprehension and diagnosis. Overall, \europa's current main strengths include: (1) expressive modeling capability; (2) flexible framework; (3) strong support for integration with other applications; (4) open-source license. It has been used for a variety of missions, mission-oriented research, and demonstrations, including: DS1: RAX ÐRemote Agent Experiment, Support for operation of the International Space Station's solar arrays, Bedrest study at Johnson Space Center, MER Tactical Activity Planning, Advanced Spaceflight Training Systems Development, MBARI's underwater autonomous vehicle, and Willow Garage's autonomous robot navigation.

Nevertheless, we still have a long list of improvements that we want to make to \europa. The most important ones in our opinion are: significantly improve search (especially heuristic guidance) and inference capabilities, support the ANML and PDDL modeling languages, improve the visualization and debugging tools and  allow \europa\ extensions to be written in other languages. Given that \europa\ is open-source software, we welcome contributions from planning and scheduling researchers and practitioners.

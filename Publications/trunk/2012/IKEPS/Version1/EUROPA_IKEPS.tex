...
\author{Javier Barreiro, Tristan Smith, Paul Morris, Jeremy Frank, Tatiana Kichkaylo, \\ add\_some\_people\_here (Ari, Conor, Michael?), Minh Do}
...
\europa\ is a class library and tool set for building and analyzing planners (and/or schedulers) within a Constraint-based Temporal Planning paradigm. This paradigm has been successfully applied in a wide range of practical planning problems and has a legacy of success in NASA applications. \europa\ offers capabilities in 3 key areas of problem solving: (1) Representation; (2) Reasoning; and (3) Search. \europa\ is a mean to integrate advanced planning, scheduling and constraint reasoning into an end-user application and is designed to be open and extendable to accommodate diverse and highly specialized problem solving techniques within a common design framework and around a common technology core.  In this paper, we will demonstrate the core capabilities of this open-source planning \& scheduling framework. While \europa\ is the complete planning and scheduling software suite, we will concentrate on the aspects that are relevant to the knowledge engineering purpose: modeling language capabilities, model debugging and execution tools, and plan visualization and analysis.
...
\europa\ (Extensible Universal Remote Operations Planning Architecture) is a class library and tool set for building planners (and/or schedulers) within a Constraint-based Temporal Planning paradigm ~\cite{frank:europa}. Constraint-based Temporal Planning1 (and Scheduling) is a paradigm of planning based on an explicit notion of time and a deep commitment to a constraint-based formulation of planning problems. This paradigm has been successfully applied in a wide range of practical planning problems and has a legacy of success in NASA applications including :
...
\europa\ is now at version 2 and is the successor of the original \europa\ which in turn was based upon HSTS ~\cite{muscettola:1998}. It has been made open-source. The source code and extensive documents on \europa\ are available at: \emph{http://code.google.com/p/europa\-pso/}. As the Planning \& Scheduling Knowledge Engineering tool, \europa\ major strengths are: (1) flexibility in integrating with client applications; (2) proven track record; (3) open-source software license; (4) very extensive online document repository with detailed guideline and a wide variety of examples in different domains.
...
From an application developerÕs view-point, the modules of interest are: {\em NDDL}, {\em Solvers} and {\em PlanWorks}. These modules address modeling, search and troubleshooting respectively. Other modules will be explored in the context of making customized extensions.
...
There are several different ways in which \europa\ can be used to to support solving a planning \& scheduling or CSP problem: (1) embed \europa\ within the client application; (2) using the provided Eclipse plugin; and (3) using \emph{PSDesktop}, a Java Swing UI Framework. For the rest of this section, we will outline those three different approaches and 


%------------------------------------
\subsection{Embed \europa\ in an Application}
\europa\ provides a \emph{makeproject} script that will generate C++ and Java projects that embed \europa, along with simple NDDL model and initial-state files that users can then modify for their own purposes. This allows the users to perform the full application cycle of:
...
\noindent {\bf Run NDDL Model Perspective:} The Run as NDDL model perspective is the Eclipse version of the JAVA Swing {\em PSDesktop} user interface, which was the different (earlier) implementation of the \europa\ user interface. The plugin can run multiple NDDL sessions at the same time. Users can switch between them using the pulldown list. \europa\ sessions are also visible in the Debug perspective and can be killed or restarted from there. Figure~\ref{fig:NDDL_runpers} shows an example of this perspective where different aspects of modeling and execution can be visually displayed.
...
\begin{figure*}
  ...
  \end{center}
  \caption{\small UI example of the classical \emph{Rovers} planning domain.}
  \label{fig:rover}
\end{figure*}
%--
...
\item Mars Ô09 - MSL : Support for planning and scheduling for Mars Science Laboratory Science Operations
...
\item Mars Ô03: MER ÐMars Exploration Rover Science Operations
...
\item Intelligent Distributed Execution Architecture (IDEA ÐEUROPA, \europa\ 2)
\item Earth-observing satellite scheduling project (EOS ÐEUROPA, \europa\ 2)
\item SOFIA flight scheduling project (SOFIA ÐEUROPA)
\item Contingent Planning for ROVER operations (PiCOÐEUROPA 2)
\item Personal Satellite Assistant (PSA ÐEUROPA)
\item Spoken Interface Prototype for PSA (RIALIST ÐEUROPA)
\item IS Milestone (\europa\ 2, support ended in 2004)
\item CDS Milestone (\europa\ 2, currently supporting)
\item DS1: RAX ÐRemote Agent Experiment (original version of technology)
\end{itemize}
